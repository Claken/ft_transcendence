...
const ProtoChat = () => {
  ...
  const toggleRoomMembership = (event: any) => {
    event.preventDefault();
    const activeRoom = findActiveRoom();
    if (activeRoom.member) socket?.emit("leaveRoom", activeRoom.name);
    else socket?.emit("joinRoom", activeRoom.name);
  };
    ...
    <div>
      <div className="chatContainer">
        <div className="full_chat">
                ...
                <tbody>
                  <tr>
                    {rooms.map((room: any, id: number) => (
                      <td key={id}>
                        <button onClick={() => setActiveForRoom(room.name)}>
                          {room.name}
                        </button>
                      </td>
                    ))}
          ...
          <div className="middle">{dmContext.haveTarget && <DmChat />}</div>
